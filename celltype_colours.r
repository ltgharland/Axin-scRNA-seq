## Harland SeqFISH Related Colours ##
embryo_colors <- c(
  "embryo_1" = "#1f78b4",  # shades of blue
  "embryo_2" = "#a6cee3",  # shades of blue
  "embryo_3" = "#33a02c",  # shades of green
  "embryo_4" = "#b2df8a",  # shades of green
  "embryo_5" = "#ff7f00",  # shades of orange
  "embryo_6" = "#fdbf6f",  # shades of orange
  "embryo_7" = "#ffcc00"   # shades of orange
)

## Stage Dataset Colours (Harland Fig. 2) ##
stage_dataset_colors = c(
"E6.5_scRNA" = "#F21A00",
"E6.5_seqFISH" = "#F21A00",
"E6.75_scRNA" = "#EC4B00",
"E7.0_scRNA" = "#E67D00",
"E7.25_scRNA" = "#E1AF00",
"E7.5_scRNA" = "#E4B80E",
"E7.5_seqFISH" = "#E4B80E",
"E7.75_scRNA"="#E7C21C",
"E8.0_scRNA" = "#EBCC2A",
"E8.25_scRNA" = "#C4C55D",
"E8.5_scRNA" = "#9EBE91",
"E8.5_seqFISH" = "#9EBE91",
"E8.75_scRNA" = "#78B7C5",
"E9.0_scRNA" = "#3399FF",
"E9.25_scRNA"="#297ACC",
"E9.5_scRNA"="#2162A3",
"Mixed gastrulation" = "#BEBEBE")

stages_colors = c(
"E6.5" = "#F21A00",
"E6.75" = "#EC4B00",
"E7.0" = "#E67D00",
"E7.25" = "#E1AF00",
"E7.5" = "#E4B80E",
"E7.75"="#E7C21C",
"E8.0" = "#EBCC2A",
"E8.25" = "#C4C55D",
"E8.5" = "#9EBE91",
"E8.75" = "#78B7C5",
"E9.0" = "#3399FF",
"E9.25"="#297ACC",
"E9.5"="#2162A3",
"Mixed gastrulation" = "#BEBEBE")

anatomy_colors = c(
"EP" = "#000000",
"YS" = "#5E4FA2",
"Anterior section" = "#774611",
"Posterior section" = "#A2D630",
"Medial section" = "#D95F02",
"Posterior"="#E2C207",
"Anterior" = "#F71616",
"Pooled" = "#BEBEBE",
"NA" = "#BEBEBE")

Pijuan_Sala_colors = c(
"Epiblast" = "#635547",
"Primitive Streak" = "#DABE99",
"Anterior Primitive Streak" = "#c19f70",
"Caudal epiblast" = "#9e6762",
"PGC" = "#FACB12",
"Anterior Primitive_Streak" = "#c19f70",
"Notochord" = "#0F4A9C",
"Def. endoderm" = "#F397C0",
"Gut" = "#EF5A9D",
"Nascent mesoderm" = "#C594BF",
"Mixed mesoderm" = "#DFCDE4",
"Intermediate mesoderm" = "#139992",
"Caudal Mesoderm" = "#3F84AA",
"Paraxial mesoderm" = "#8DB5CE",
"Somitic mesoderm" = "#005579",
"Pharyngeal mesoderm" = "#C9EBFB",
"Cardiomyocytes" = "#B51D8D",
"Allantois" = "#532C8A",
"ExE mesoderm" = "#8870ad",
"Mesenchyme" = "#cc7818",
"Haematoendothelial progenitors" = "#FBBE92",
"Endothelium" = "#ff891c",
"Blood progenitors" = "#c9a997",
"Blood progenitors 1" = "#f9decf",
"Blood progenitors 2" = "#c9a997",
"Erythroid" = "#EF4E22",
"Erythroid1" = "#C72228",
"Erythroid2" = "#f79083",
"Erythroid3" = "#EF4E22",
"NMP" = "#8EC792",
"Neurectoderm" = "#65A83E",
"Rostral neurectoderm" = "#65A83E",
"Caudal neurectoderm" = "#354E23",
"Neural crest" = "#C3C388",
"Forebrain Midbrain_Hindbrain" = "#647a4f",
"Spinal cord" = "#CDE088",
"Surface ectoderm" = "#f7f79e",
"Visceral endoderm" = "#F6BFCB",
"ExE endoderm" = "#7F6874",
"ExE ectoderm" = "#989898",
"Parietal endoderm" = "#1A1A1A"
)

Imaz_Rosshandler_colors = c(
"Epiblast" = "#635547",
"Epiblast_0" = "#635547",
"Epiblast_1" = "#FACB12",
"Primitive Streak" = "#DABE99",
"Caudal epiblast" = "#9e6762",
"PGC" = "#FACB12",
"Anterior Primitive Streak" = "#c19f70",
"Node"="#153b3d",
"Notochord" = "#0F4A9C",
"Gut tube" = "#EF5A9D",
"Hindgut" = "#F397C0",
"Midgut" = "#ff00b2",
"Foregut" = "#ffb7ff",
"Pharyngeal endoderm"="#95e1ff",
"Thyroid primordium"="#97bad3",
"Nascent mesoderm" = "#C594BF",
"Intermediate mesoderm" = "#139992",
"Caudal mesoderm" = "#3F84AA",
"Lateral plate mesoderm" = "#F9DFE6",
"Limb mesoderm" = "#e35f82",
"Forelimb" = "#d02d75",
"Kidney primordium" = "#e85639",
"Presomitic mesoderm"="#5581ca",#"#0000ff",#blue
"Somitic mesoderm" = "#005579",
"Posterior somitic tissues" = "#5adbe4",#"#40e0d0",#turquoise
"Paraxial mesoderm" = "#8DB5CE",
"Cranial mesoderm" = "#456722",#"#006400",#darkgreen
"Anterior somitic tissues"= "#d5e839",
"Sclerotome" = "#e3cb3a",#"#ffff00",#yellow
"Dermomyotome" = "#00BFC4",#"#a52a2a",#brown
"Pharyngeal mesoderm" = "#C9EBFB",
"Cardiopharyngeal progenitors" = "#556789",
"Anterior cardiopharyngeal progenitors"="#683ed8",
"Allantois" = "#532C8A",
"Mesenchyme" = "#cc7818",
"YS mesothelium" = "#ff7f9c",
"Epicardium"="#f79083",
"Embryo proper mesothelium" = "#ff487d",
"Cardiopharyngeal progenitors FHF"="#d780b0",
"Cardiomyocytes FHF 1"="#a64d7e",
"Cardiomyocytes FHF 2"="#B51D8D",
"Cardiopharyngeal progenitors SHF"="#4b7193",
"Cardiomyocytes SHF 1"="#5d70dc",
"Cardiomyocytes SHF 2"="#332c6c",
"Haematoendothelial progenitors" = "#FBBE92",
"Blood progenitors" = "#6c4b4c",
"Erythroid" = "#C72228",
"Chorioallantoic-derived erythroid progenitors"="#E50000",
"Megakaryocyte progenitors"="#e3cb3a",
"MEP"="#EF4E22",
"EMP"="#7c2a47",
"YS endothelium"="#ff891c",
"YS mesothelium-derived endothelial progenitors"="#AE3F3F",
"Allantois endothelium"="#2f4a60",
"Embryo proper endothelium"="#90e3bf",
"Venous endothelium"="#bd3400",
"Endocardium"="#9d0049",
"NMPs/Mesoderm-biased" = "#89c1f5",
"NMPs" = "#8EC792",
"Ectoderm" = "#ff675c",
"Optic vesicle" = "#bd7300",
"Ventral forebrain progenitors"="#a0b689",
"Early dorsal forebrain progenitors"="#0f8073",
"Late dorsal forebrain progenitors"="#7a9941",
"Midbrain/Hindbrain boundary"="#8ab3b5",
"Midbrain progenitors"="#9bf981",
"Dorsal midbrain neurons"="#12ed4c",
"Ventral hindbrain progenitors"="#7e907a",
"Dorsal hindbrain progenitors"="#2c6521",
"Hindbrain floor plate"="#bf9da8",
"Hindbrain neural progenitors"="#59b545",
"Neural tube"="#233629",
"Migratory neural crest"="#4a6798",
"Branchial arch neural crest"="#bd84b0",
"Frontonasal mesenchyme"="#d3b1b1",
"Spinal cord progenitors"="#6b2035",
"Dorsal spinal cord progenitors"="#e273d6",
"Non-neural ectoderm" = "#f7f79e",
"Surface ectoderm" = "#fcff00",
"Epidermis" = "#fff335",
"Limb ectoderm" = "#ffd731",
"Amniotic ectoderm" = "#dbb400",
"Placodal ectoderm" = "#ff5c00",
"Otic placode"="#f1a262",
"Otic neural progenitors"="#00b000",
"Visceral endoderm" = "#F6BFCB",
"ExE endoderm" = "#7F6874",
"ExE ectoderm" = "#989898",
"Parietal endoderm" = "#1A1A1A",
"Endotome" = "#808080"
)

Harland_colors <- c(
"Epiblast" = "#635547",
"Primitive Streak" = "#DABE99",
"Caudal epiblast" = "#9E6762",
"PGC" = "#FACB12",
"Anterior Primitive Streak" = "#C19F70",
"Node"="#153B3D",
"Notochord" = "#0F4A9C",
"Gut tube" = "#EF5A9D",
"Hindgut" = "#F397C0",
"Midgut" = "#FF00B2",
"Foregut" = "#FFB7FF",
"Pharyngeal endoderm"="#95E1FF",
"Thyroid primordium"="#97BAD3",
"Nascent mesoderm" = "#C594BF",
"Intermediate mesoderm" = "#139992",
"Caudal mesoderm" = "#3F84AA",
"Limb mesoderm" = "#E35F82",
"Kidney primordium" = "#E85639",
"Presomitic mesoderm"="#5581CA",#"#0000FF",#blue
"Somitic mesoderm" = "#005579",
"Posterior somitic tissues" = "#5ADBE4",#"#40E0D0",#turquoise
"Paraxial mesoderm" = "#8DB5CE",
"Cranial mesoderm" = "#456722",#"#006400",#darkgreen
"Anterior somitic tissues"= "#D5E839",
"Sclerotome" = "#E3CB3A",#"#FFFF00",#yellow
"Dermomyotome" = "#00BFC4",#"#A52A2A",#brown
"Pharyngeal mesoderm" = "#C9EBFB",
"Anterior cardiopharyngeal progenitors"="#683ED8",
"Allantois" = "#532C8A",
"YS mesothelium" = "#FF7F9C",
"Epicardium"="#F79083",
"Embryo proper mesothelium" = "#FF487D",
"Cardiopharyngeal progenitors FHF"="#D780B0",
"Cardiomyocytes FHF 1"="#A64D7E",
"Cardiomyocytes FHF 2"="#B51D8D",
"Cardiomyocytes SHF 1"="#5D70DC",
"Cardiomyocytes SHF 2"="#332C6C",
"Haematoendothelial progenitors" = "#FBBE92",
"Blood progenitors" = "#6C4B4C",
"Erythroid" = "#C72228",
"Chorioallantoic-derived erythroid progenitors"="#E50000",
"Megakaryocyte progenitors"="#E3CB3A",
"MEP"="#EF4E22",
"YS endothelium"="#FF891C",
"YS mesothelium-derived endothelial progenitors"="#AE3F3F",
"Allantois endothelium"="#2F4A60",
"Embryo proper endothelium #1" = "#69B8A5",  # Original color
"Embryo proper endothelium #2" = "#B2DBCE",  # Much lighter shade of the original color
"Endocardium #1" = "#FF6666",  # Much lighter shade
"Endocardium #2" = "#FFB3B3",  # Medium light shade
"EMP" = "#5A0026",  # Darker than both Endocardium shades
"Venous endothelium"="#BD3400",
"NMPs/Mesoderm-biased" = "#89C1F5",
"NMPs" = "#8EC792",
"Ectoderm" = "#FF675C",
"Optic vesicle" = "#BD7300",
"Ventral forebrain progenitors"="#A0B689",
"Midbrain/Hindbrain boundary"="#8AB3B5",
"Midbrain progenitors"="#9BF981",
"Dorsal midbrain neurons"="#12ED4C",
"Ventral hindbrain progenitors"="#7E907A",
"Dorsal hindbrain progenitors"="#2C6521",
"Hindbrain floor plate"="#BF9DA8",
"Hindbrain neural progenitors"="#59B545",
"Neural tube"="#233629",
"Migratory neural crest"="#4A6798",
"Branchial arch neural crest"="#BD84B0",
"Frontonasal mesenchyme"="#D3B1B1",
"Spinal cord progenitors"="#6B2035",
"Dorsal spinal cord progenitors"="#E273D6",
"Non-neural ectoderm" = "#F7F79E",
"Surface ectoderm" = "#FCFF00",
"Epidermis" = "#FFF335",
"Limb ectoderm" = "#FFD731",
"Amniotic ectoderm" = "#DBB400",
"Placodal ectoderm" = "#FF5C00",
"Otic placode"="#F1A262",
"Otic neural progenitors"="#00B000",
"Visceral endoderm" = "#F6BFCB",
"ExE endoderm" = "#7F6874",
"Parietal endoderm" = "#1A1A1A",
"Cranial mesoderm and anterior somitic tissue" = "#808080",
"ExE ectoderm proximal" = "#696969",
"ExE ectoderm distal" = "#A0A0A0",
"Anterior LPM"="#F9DFE6",
"ExE mesoderm" = "#956D91",              # Purple
"ExE mesoderm and Anterior LPM" = "#D48A8D",    # Medium Purple
"Mesenchyme" = "#CC7818",                # Existing color
"ExE mesoderm and Posterior LPM" = "#9B81CF", # Blue Violet
"Rostral ectoderm" = "#A48F73",
"Endotome" = "#808080"
)

Rossi_colors <- c(
  "Hematoendothelial progenitors" = "#ff6efd",
  "Endothelium" = "#ff5075",
  "Mesenchyme" = "#ee5151",
  "Cardiomyocytes" = "#ff0000",
  "Pharyngeal mesoderm" = "#f8e16c",
  "Mixed mesoderm" = "#df8888",
  "Nascent mesoderm" = "#d9b2b2",
  "Paraxial mesoderm" = "#f7875b",
  "Somitic mesoderm" = "#cc5252",
  "Differentiation front" = "#e1694c",
  "Somite/Sclerotome" = "#8d6f5f",
  "Intermediate mesoderm" = "#bc9962",
  "Caudal Mesoderm" = "#e4ace3",
  "NMPs" = "#427aa6",
  "NMPs (mesoderm)" = "#af5c65",
  "Caudal epiblast" = "#7cb1c0",
  "Surface ectoderm" = "#15f0c1",
  "Oral ectoderm" = "#29d99d",
  "Rostral neuroectoderm" = "#45ebec",
  "Epiblast" = "#cdcdcd",
  "Primitive streak" = "#858585",
  "Anterior primitive streak" = "#cadebc",
  "Definitive endoderm" = "#8ea67c",
  "Notochord" = "#217346",
  "Node" = "#dbe789",
  "Gut/visceral endoderm" = "#c3c45c",
  "Gut" = "#7ceb52",
  "Hindgut" = "#b0f777",
  "Foregut" = "#77f7a1",
  "Early cardiomyocytes" = "#ee5151",
  "Early mesoderm" = "#cc5252",
  "Differentiated somite" = "#f7875b",
  "Cardiac mesoderm" = "#df8888",
  "Caudal mesoderm" = "#e4ace3",
  #"Radial glial-like/Neural crest" = "#86d0ec",
  "Forebrain/Midbrain/Hindbrain/NeuralCrest" = "#86d0ec",
  "Differentiated neurons" = "#1aa6ff",
  "Spinal cord" = "#609fd1",
  "Definitive endoderm" = "#8ea67c"
)

# SeqFISH Cell Type Colours 

Harland_seqFISH <- c(
    ## failed QC
  "low quality" = "#E0E0E0",
  
    #e6 and e7 cell types 
   # Ectoderm (shades of blue and purple)
  "distal ExE ectoderm #1" = "#8A2BE2",   # Blue Violet
  "distal ExE ectoderm #2" = "#7B68EE",   # Medium Slate Blue


  # Epiblast, Rostral Epiblast, and Primitive Streak (shades of brown)
  "epiblast" = "#8B4513",        # Saddle Brown
  "rostral epiblast" = "#A0522D",   # Sienna
  "caudal epiblast" = "#8B0000",     # Dark Red (distinct from brown tones)

  # Endoderm (shades of green, except for definitive endoderm in pink)
  "visceral endoderm #1" = "#6B8E23",   # Olive Drab
  "visceral endoderm #2" = "#32CD32",   # Lime Green
  "ExE endoderm" = "#228B22",           # Forest Green
    
  "anterior primitive streak" = "#ADD8E6",  # Light Blue

  # Mesoderm (shades of orange and yellow)
  "primitive streak" = "#FFD700",   # Gold (Bright Yellow)
  "ExE mesoderm" = "#FF8C00",       # Dark Orange
  "mesodermal wings" = "#FFA07A",   # Light Salmon (Pale Orange)
  "haematoendothelial #1" = "#FF4500",  # Orange Red (Bright Orange-Red)
  "haematoendothelial #2" = "#B22222",  # Firebrick
  
  # Proximal Ectoderm Cells (shades of pink)
  "proximal ExE ectoderm #1" = "#FF1493",   # Deep Pink
  "proximal ExE ectoderm #2" = "#FF69B4",   # Hot Pink (used previously for definitive endoderm, so you might consider another shade like Medium Violet Red)
    
  "endothelium #1" = "#3CB371",    # Medium Sea Green
  "spinal cord" = "#9400D3",    # Dark Violet
  "blood" = "#FF6347",    # Tomato
  "anterior somitic tissue" = "#FFD700",    # Gold
  "lateral plate mesoderm" = "#4169E1",    # Royal Blue
  "brain #1" = "#FF0000",    # Red
  "posterior somitic tissue" = "#87CEEB",    # Sky Blue
  "brain #2" = "#A9A9A9",    # Dark Grey
  "brain #3" = "#FF69B4",    # Hot Pink
  "cardiomyocytes" = "#FF8C00",    # Dark Orange
  "posterior somatic LP mesoderm #2" = "#8A2BE2",   # Blue Violet
  "hindgut" = "#800000",   # Maroon
  "midgut" = "#556B2F",   # Dark Olive Green
  "neural crest #1" = "#8B4513",   # Saddle Brown
  "mesenchyme" = "#FF6347",   # Tomato
  "paraxial mesoderm" = "#9932CC",   # Dark Orchid
  "surface ectoderm #1" = "#4169E1",   # Royal Blue
  "presomitic mesoderm" = "#3CB371",   # Medium Sea Green
  "surface ectoderm #2" = "#FF69B4",   # Hot Pink
  "neural crest #2" = "#FF0000",   # Red
  "brain #6" = "#FFD700",   # Gold
  "posterior somatic LP mesoderm #1" = "#E6E6FA",   # Lavender
  "dorsal foregut" = "#32CD32",   # Lime Green
  "splanchnic LP mesoderm #1" = "#8B4513",   # Saddle Brown
  "splanchnic LP mesoderm #2" = "#FFB6C1",   # Light Pink
  "allantois" = "#FFD700",   # Gold
  "ventral foregut" = "#87CEEB",   # Sky Blue
  "brain #5" = "#FFD700",   # Gold
  "endothelium #2" = "#FFA500",   # Orange
  "neural crest #3" = "#556B2F",   # Dark Olive Green
  "brain #4" = "#FF8C00",   # Dark Orange
  "otic placode" = "#800080",   # Purple
  "macrophage" = "#808080"    # Gray
)

seqFISH_refined_annotation_colors <- c(
    # Epiblast and primitive streak populations
    "Epiblast" = "#635547",
    "Primitive Streak" = "#DABE99",
    "Rostral Ectoderm" = "#A48F73",
    "Caudal Epiblast" = "#9e6762",
    
    # Endodermal populations
    "Definitive Endoderm" = "#FFA500",  # Orange (Standard)
    "ExE Endoderm" = "#7F6874",
    "Visceral Endoderm #1" = "#F6BFCB",
    "Visceral Endoderm #2" = "#E89BAF",  # Slightly darker pinkish shade
    
    # Mesodermal populations
    "Axial Mesendoderm" = "#228B22",  # Green (Forest Green)
    "Mesodermal Wings" = "#3F84AA",
    "ExE Mesoderm" = "#956D91",  
    
    # Haematoendothelial populations
    "Haematoendothelial Progenitors #1" = "#FBBE92",  # Light Peach
    "Haematoendothelial Progenitors #2" = "#E69C72",  # Darker Peach/Salmon
    
    # Extraembryonic ectoderm
    "ExE Ectoderm Proximal #1" = "#A0A0A0",  # Light Gray
    "ExE Ectoderm Proximal #2" = "#888888",  # Medium-Light Gray
    "ExE Ectoderm Distal #1" = "#505050",  # Dark Gray
    "ExE Ectoderm Distal #2" = "#606060",   # Slightly Lighter Dark Gray
    "low quality" = "#E0E0E0", 
    
    # Low-quality/mixed classification
    "LQ" = "#B0B0B0"
    
)

Harland_seqFISH_early <- c(
    ## failed QC
  "low quality" = "#E0E0E0",
  
    #e6 and e7 cell types 
   # Ectoderm (shades of blue and purple)
  "ExE Ectoderm Distal #1" = "#8A2BE2",   # Blue Violet
  "ExE Ectoderm Distal #2" = "#7B68EE",   # Medium Slate Blue


  # Epiblast, Rostral Epiblast, and Primitive Streak (shades of brown)
  "Epiblast" = "#8B4513",        # Saddle Brown
  "Rostral Ectoderm" = "#D2691E",   # Sienna
  "Caudal Epiblast" = "#8B0000",     # Dark Red (distinct from brown tones)

  # Endoderm (shades of green, except for definitive endoderm in pink)
  "Visceral Endoderm #1" = "#6B8E23",   # Olive Drab
  "Visceral Endoderm #2" = "#32CD32",   # Lime Green
  "ExE Endoderm" = "#4169E1",  
    
  #"anterior primitive streak" = "#ADD8E6",  # Light Blue
  "Axial Mesendoderm" = "#ADD8E6",  # Green (Forest Green)
  "Definitive Endoderm" = "#000000",  # Orange (Standard)

  # Mesoderm (shades of orange and yellow)
  "Primitive Streak" = "#FFD700",   # Gold (Bright Yellow)
  "ExE Mesoderm" = "#FF8C00",       # Dark Orange
  "Mesodermal Wings" = "#FFA07A",   # Light Salmon (Pale Orange)
  "Haematoendothelial Progenitors #1" = "#FF2400",  # Orange Red (Bright Orange-Red)
  "Haematoendothelial Progenitors #2" = "#B22222",  # Firebrick
  
  # Proximal Ectoderm Cells (shades of pink)
  "ExE Ectoderm Proximal #1" = "#FF1493",   # Deep Pink
  "ExE Ectoderm Proximal #2" = "#FF69B4"   # Hot Pink (used previously for definitive endoderm, so you might consider another shade like Medium Violet Red)
    )

Hernandez_colors <- c(
  "ExE Ectoderm" = "#989898",                      # Red
  "ExE Endoderm" = "#7F6874",                      # Orange
  "Visceral Endoderm" = "#F6BFCB",                 # Yellow
  "Epiblast" = "#635547",                          # Green
  "Primitive Streak" = "#DABE99",                  # Cyan
  "Anterior Primitive Streak" = "#FFA500",         # Orange
  #"Nascent mesoderm #1" = "#C594BF",               # Magenta
  #"Nascent mesoderm #2" = "#8C5A82",               # Darker Purple
  "Nascent mesoderm #1" = "#DDA0DD",               # Magenta
  "Nascent mesoderm #2" = "#5E2D79",               # Darker Purple
  "Haematoendothelial progenitors #1" = "#FBBE92", # Rose
  "Haematoendothelial progenitors #2" = "#6c4b4c", # Chartreuse
  "Posterior Primitive Streak" = "#5A5AC5",        # Soft Blue
  "Posterior Lateral Plate Mesoderm" = "#7F0000",  # Maroon
  #"Mesenchyme" = "#cc7818",                        # Azure
  "Mesenchyme" = "#e66100",        
  "Allantois Mesoderm" = "#3D99C3",                # Medium Blue
  "Intermediate Mesoderm" = "#139992",             # Spring Green
  #"Low Quality" = "#808080",# Gray
  "Low Quality" = "#D9D9D9",      
  "Parietal Endoderm" = "#1A1A1A"                  # Black
)

Hernandez_dataset_colors = c(
"Gastrulation Atlas" = "#B0B0B0",
"Axin Control" = "#55BCC2",
"Axin DKO" = "#E87D72")

Hernandez_stages_colors = c(
  "E6.5" = "#FF0000",       # Red
  "E6.75" = "#FF4500",      # Orange-Red
  "E7.0" = "#FFA500",       # Orange
  "E7.25" = "#FFD700",      # Gold
  "E7.5" = "#FFFF00",       # Yellow
  "E7.75" = "#00BFFF",      # Deep Sky Blue
  "E8.0" = "#1E90FF",       # Dodger Blue
  "E8.25" = "#4169E1",      # Royal Blue
  "E8.5" = "#800080"        # Purple
)

Hernandez_colors_filtered <- c(
  "ExE Ectoderm" = "#989898",                      # Red
  "ExE Endoderm" = "#7F6874",                      # Orange
  "Visceral Endoderm" = "#F6BFCB",                 # Yellow
  "Parietal Endoderm" = "#1A1A1A",                  # Black
  "Epiblast" = "#635547",                          # Green
  "Primitive Streak" = "#DABE99",                  # Cyan
  "Anterior Primitive Streak" = "#FFA500",         # Orange
  #"Nascent mesoderm #1" = "#C594BF",               # Magenta
  #"Nascent mesoderm #2" = "#8C5A82",               # Darker Purple
  "Nascent mesoderm #1" = "#DDA0DD",               # Magenta
  "Nascent mesoderm #2" = "#5E2D79",               # Darker Purple
  "Haematoendothelial progenitors #1" = "#FBBE92", # Rose
  "Haematoendothelial progenitors #2" = "#6c4b4c", # Chartreuse
  "Posterior Lateral Plate Mesoderm" = "#7F0000",  # Maroon
  #"Mesenchyme" = "#cc7818",                        # Azure
  "Mesenchyme" = "#e66100",        
  "Posterior Primitive Streak" = "#5A5AC5",        # Soft Blue
  "Allantois Mesoderm" = "#3D99C3",                # Medium Blue
  "Intermediate Mesoderm" = "#139992"             # Spring Green
  #"Low Quality" = "#808080",# Gray
  #"Low Quality" = "#D9D9D9",      
)
